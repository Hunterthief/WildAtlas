...
async function loadModelLinks() {
  try {
    const response = await fetch('data/model_links.json');
    if (response.ok) {
...
function initHomePage() {
  ...
  fetchAnimals()
    ...
    .catch(error => {
      ...
      showErrorState('Failed to load animal data. Please refresh the page.');
    });
...
function renderAnimalCard(animal, type) {
  ...
  card.setAttribute('aria-label', `View details for ${animal.name}`);
  ...
  card.addEventListener('click', navigateToAnimal);
...
async function initDetailPage() {
  ...
  // Show loading state
  document.getElementById('animal-name').textContent = 'Loading...';
  ...
  try {
    ...
    if (!animal) {
      ...
      document.getElementById('overview-text').textContent = 'The animal you\'re looking for doesn\'t exist in our database.';
      return;
    }
    ...
    document.getElementById('animal-name').textContent = 'Error Loading Data';
    document.getElementById('overview-text').textContent = 'Failed to load animal information. Please try again later.';
  }
}
...
function populateDetailPage(animal) {
  ...
  if (dietIcons && eco.diet) {
    ...
    dietIcons.innerHTML = dietTypes.map(type => `
      <div class="diet-icon ${type.class}" title="${type.title}">
        ${type.icon}
      </div>
    `).join('');
  }
