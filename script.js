...
async function setup3DModel(animal) {
    ...
    let modelUrl = findMatchingModel(animal.name);
    
    // No 3D model found - show image only
    if (!modelUrl) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Use model-viewer with glTF/GLB URL if available
    ...
    // Fallback to image if oEmbed fails
    imageElement.style.display = 'block';
    modelViewer.style.display = 'none';
    if (toggleButton) toggleButton.style.display = 'none';
}
...
console.log('🌍 WildAtlas - Discover the Animal Kingdom');// Location coordinates for world map (percentage-based for responsive SVG)
const locationCoordinates = {
    'asia': { x: 75, y: 40 },
    'china': { x: 78, y: 42 },
    'india': { x: 72, y: 50 },
