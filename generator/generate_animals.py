...
def extract_locations(text, animal_type):
    """Extract locations - FIXED with proper filtering"""
    if not text: 
        return None
    ...
    for loc in locs:
        loc_lower = loc.lower()
        # Filter out obviously wrong locations based on animal type
        ...
        if animal_type == 'penguin' and any(w in loc_lower for w in ['asia', 'africa', 'north america']):
            continue  # Penguins are southern hemisphere
...
def extract_behavior(text, animal_type):
    """Extract behavior - FIXED with animal-specific logic"""
    if not text: 
        return None
